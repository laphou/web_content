...
if (isset($_POST['email'])) {
    ...
    if (mysqli_num_rows($result) > 0) { // Check if the email exists in the database
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $username = $row["username"] . ""; // Get the username of the user who requested reset
        }
        ...
        if (mysqli_query($con, sqlonef($email, $token))) {  // Add the entry of password reset to the database

            $subject = "Reset your Password"; // Subject for sending email to user

            $reset_link = "http://localhost/GetSimpleCMS-3.3.16/reset.php" . "?token=" . $token . ""; // The password reset link which user will recieve in email

            $url = "http://localhost/GetSimpleCMS-3.3.16/email.php";
            $data = array('username' => $username, 'email' => $email, 'reset_link' => $reset_link);
